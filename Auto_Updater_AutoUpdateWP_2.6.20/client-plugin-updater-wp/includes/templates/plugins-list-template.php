...
delete_transient('plugin_updater_available_plugins');

// Obtener plugins disponibles directamente del servidor
...
// Verificar si la respuesta es válida y contiene plugins
if (
    empty($plugins) ||
    !is_array($plugins) ||
    (isset($plugins->error) && !empty($plugins->error)) ||
    (is_string($plugins) && !empty($plugins))
) {
    // Si el servidor devuelve un mensaje de error, mostrarlo
    $error_message = '';
    if (is_object($plugins) && isset($plugins->error)) {
        $error_message = esc_html($plugins->error);
    } elseif (is_string($plugins) && !empty($plugins)) {
        $error_message = esc_html($plugins);
    } else {
        $error_message = 'No se encontraron plugins disponibles. Por favor, verifica tu API Key o contacta con soporte.';
    }
...
// Obtener plugins con auto-actualización activada
$auto_update_plugins = (array) get_site_option('auto_update_plugins', []);
?>

<div class="wrap plugin-updater-wrap">
    <h1><?php echo esc_html($white_label_name); ?> Plugins AutoUpdate AutoUpdate-Wp </h1>
    <div class="plugin-updater-info">
        <p>Desde aquí puedes instalar y actualizar los plugins disponibles con tu API Key.</p>
    </div>
        ...
        <table class="plugin-updater-table">
            ...
            <tbody>
                <?php foreach ($plugins as $plugin): ?>
                    <?php
                    // Verificar que $plugin sea un objeto válido con las propiedades necesarias
                    if (!is_object($plugin) || !isset($plugin->slug) || !isset($plugin->version)) {
                        continue;
                    }
                    
                    // Procesar datos del plugin de forma segura
                    $plugin_slug = isset($plugin->slug) ? (string)$plugin->slug : '';
                    $server_slug = strtolower(trim($plugin_slug));
                    $is_installed = isset($installed_slugs[$server_slug]);
                    $installed_version = $is_installed ? $installed_slugs[$server_slug]['version'] : '';
                    $server_version = isset($plugin->version) ? trim((string)$plugin->version) : '';
                    $plugin_name = isset($plugin->name) ? (string)$plugin->name : 'Sin nombre';
                    ...
                    $plugin_file = $is_installed ? $installed_slugs[$server_slug]['file'] : '';
                    $is_auto_update = $is_installed && in_array($plugin_file, $auto_update_plugins);
                    ?>
                    <tr>
                        <td class="plugin-name"><strong><?php echo esc_html($plugin_name); ?></strong></td>
                        ...
                        <td class="plugin-action">
                            <?php if ($action_text !== 'Instalado'): ?>
                                <a href="#" class="button <?php echo esc_attr($action_class); ?>"
                                   ...
                                   data-file="<?php echo $plugin_file; ?>">
                        ...
                        <td class="plugin-auto-update">
                            <?php if ($is_installed): ?>
                                <button class="button toggle-auto-update" 
...
jQuery(document).ready(function($) {
    // Actualizar/instalar plugin (con timeout largo)
    $('.install-now, .update-now').on('click', function(e) {
        ...
        var data = {
            'action': 'plugin_updater_manage_plugin',
            'slug': slug,
    ...
    $('.toggle-auto-update').on('click', function(e) {
        e.preventDefault();
        var $btn = $(this);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'plugin_updater_toggle_auto_update',
                plugin: plugin,
...
<style>
/* Tus estilos originales aquí, no los cambio */
.plugin-updater-wrap {
    max-width: 1200px;
    margin: 20px auto;
    font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", sans-serif;
    background: linear-gradient(135deg, #FF6B6B, #DA70D6);
    padding: 20px;
    border-radius: 12px;
